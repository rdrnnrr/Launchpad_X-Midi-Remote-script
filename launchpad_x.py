...
class Launchpad_X(InstrumentControlMixin, NovationBase):
    ...
    def _create_components(self):
        ...
        # Add listeners for arrow buttons
        self.__on_up_button_value.subject = self._elements.up_button
        self.__on_down_button_value.subject = self._elements.down_button
        self.__on_left_button_value.subject = self._elements.left_button
        self.__on_right_button_value.subject = self._elements.right_button

    @listens('value')
    def __on_up_button_value(self, value):
        if value:
            # Scroll up in the Session view
            self.application.view.scroll_view(0, 'Session', False)

    @listens('value')
    def __on_down_button_value(self, value):
        if value:
            # Scroll down in the Session view
            self.application.view.scroll_view(1, 'Session', False)

    @listens('value')
    def __on_left_button_value(self, value):
        if value:
            # Scroll left in the Session view (move to previous track)
            self.application.view.scroll_view(2, 'Session', False)

    @listens('value')
    def __on_right_button_value(self, value):
        if value:
            # Scroll right in the Session view (move to next track)
            self.application.view.scroll_view(3, 'Session', False)
            ...
            self._mixer_modes.add_mode(
                ...
                (
                    partial(
                        ...
                        if is_pan
                        else sysex.FADER_VERTICAL_ORIENTATION,
    ...
    def _create_session_modes(self):
        ...
        self._session_modes.add_mode(
            'launch',
            AddLayerMode(self._session, Layer(scene_launch_buttons='scene_launch_buttons')),
        )
        self._session_modes.add_mode(
    ...
    def _create_main_modes(self):
        self._main_modes = ModesComponent(
            ...
            is_enabled=False,
            layer=Layer(
                session_button='session_mode_button',
                note_button='note_mode_button',
                custom_button='custom_mode_button',
            ),
        )
        self._main_modes.add_mode(
            'session', (self._session_modes), behaviour=(ImmediateBehaviour())
        )
        self._main_modes.add_mode(
            'note', (self._note_modes), behaviour=(ImmediateBehaviour())
        )
    ...
    def _drum_group_changed(self):
        ...
        self._elements.note_layout_switch.send_value(
            sysex.DRUM_LAYOUT_BYTE if drum_group_valid else sysex.SCALE_LAYOUT_BYTE
        )
